function coloringEdge(celAutom::CellulaireAutomaat)
    ...
    for edge in collect(edges(celAutom.mg))
        ...
        elif (ltransition + htransition)/dx < 0.2
            colors[j]= colorant"grey"
        end
        elif (ltransition + htransition)/dx < 0.4
            colors[j]= colorant"blue"
        end
        elif (ltransition + htransition)/dx < 0.6
            colors[j]= colorant"green"
        end
        elif (ltransition + htransition)/dx < 0.8
            colors[j]= colorant"orange"
        end
        elif (ltransition + htransition)/dx == 1
